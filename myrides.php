            ...
            <div id="articleDiv">
                <article id="contentArticle">
                    ...
                    <section id="contentSection">
						...
						if(isset($_POST['removesubmit']) AND $_POST['removeride']){
							$sql="SELECT * FROM rides WHERE id='$_POST[removeride]'";
							if(!mysqli_query($conn,$sql))
								die("Error occured while deleting Ride".mysqli_error($conn)." <a href='index.php' class='gohome'> Go to Homepage </a>");

							$sql="DELETE FROM rides WHERE id='$_POST[removeride]'";
							if(mysqli_query($conn,$sql))
								echo "Ride has been successfully deleted<br>";
							else
								die("Error occured while deleting Ride".mysqli_error($conn)." <a href='index.php' class='gohome'> Go to Homepage </a>");
						}
						$sql="SELECT * FROM rides WHERE username='$usern'";	
							$result=mysqli_query($conn,$sql);
							if(mysqli_num_rows($result)){
								while($row=mysqli_fetch_assoc($result)){
									echo $row["id"]." ".$row["name"]." ".$row["phonenumber"]." ".$row["startp"]." ".$row["endp"]." ".$row["dateofride"]." ".$row["timeofride"]." ".$row["capacity"]."<br>";
								}
								echo "<br><br>Write an index of ride that you want to remove<br>"."
									<form action='myrides.php' method='POST'>
									<input type='number' name='removeride'>
									<input type='submit' name='removesubmit'>
								";

							}
							else{
								echo "You had no rides";
							}
							mysqli_close($conn);
						?>
